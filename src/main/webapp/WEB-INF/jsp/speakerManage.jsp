				...
				<tbody>
                   ...
                   <c:forEach items="${speaker}" var="i">
					<tr class="checkall">
						...
						<td id="id">${i.id }</td>
						<td id="course_title">${i.speaker_name}</td>
						<td>${i.speaker_job}</td>
						<td
							style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">${i.speaker_desc}</td>
						<td><a href="updateSpeaker.do?id=${i.id}">✎</a></td>
						<td><a href="deleteSpeaker.do?id=${i.id}" id="delete">X</a></td>
					</tr>
				</c:forEach>	

				<tr>
						<td colspan="7"><font>总共${count}条,当前第${page }页</font> 
						<c:if test="${count%5==0}">
	...
	<script>
		...
		function deleteAll(){		
			...
			if(a==true){
				...
				location.href="deleteAllSpeaker.do?str="+str;
			}else{
